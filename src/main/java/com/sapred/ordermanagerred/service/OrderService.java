...
public class OrderService {
    ...
    public void fill() {
        ...
        for (int i = 200; i < 500; i++) {
            if (i % 3 == 0)
                orders.add(new Order(Integer.toString(i), "employee", "customer", i * 2, new Company("11", "333", 88, d), new AuditData(LocalDate.now(), /*new Date(i * 1000)*/LocalDate.now()), "1"));
            else if (i % 3 == 1)
                orders.add(new Order(Integer.toString(i), "custumer", "customer", i * 2, new Company("22", "333", 88, d), new AuditData(LocalDate.now(),LocalDate.now() /*new Date(i * 1000)*/), "2"));
            else
                orders.add(new Order(Integer.toString(i), "111", "customer", i * 2, new Company("11", "333", 88, d), new AuditData(LocalDate.now(),LocalDate.now() /*new Date(i * i)*/), "3"));
        }
